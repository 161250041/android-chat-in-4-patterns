...
@Log
public class Hw1TalkActivity extends AppCompatActivity implements nju.androidchat.client.mvp0.Mvp0Contract.View, TextView.OnEditorActionListener, OnRecallMessageRequested {
    private nju.androidchat.client.mvp0.Mvp0Contract.Presenter presenter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nju.androidchat.client.mvp0.Mvp0TalkModel mvp0TalkModel = new Mvp0TalkModel();
        ...
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        if (message.getSenderUsername().equals(this.presenter.getUsername())) {
                            if (isImage) {
                                content.addView(new ItemImageSend(this, text, message.getMessageId()));
                            } else {
                                content.addView(new ItemTextSend(this, text, message.getMessageId(), this));
                            }
                        } else {
                            if (isImage) {
                                content.addView(new ItemImageReceive(this, text, message.getMessageId()));
                            } else {
